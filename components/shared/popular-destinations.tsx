...
export function PopularDestinations({ places }: PopularDestinationsProps) {
      ...
      <div className="relative">
        <HorizontalScroll className="items-start gap-4 pb-7 pt-1" showControls itemsPerStep={6} bleed={false}>
          {places.map((place) => (
            <Link
              key={place.id}
              href={`/tours?q=${encodeURIComponent(place.query)}`}
              className="group relative block shrink-0 w-[52vw] max-w-[280px] sm:w-[42vw] md:w-[260px] lg:w-[calc((100%-5rem)/6)] lg:max-w-none xl:w-[calc((100%-5rem)/6)] 2xl:w-[calc((100%-5rem)/6)]"
            >
              ...
              <div className="relative overflow-hidden rounded-[1.5rem] bg-surface shadow-[0_24px_42px_-24px_rgba(15,23,42,0.75)] transition-transform duration-300 group-hover:-translate-y-0.5">
                <div className="relative aspect-[2/3] w-full overflow-hidden rounded-[1.5rem] bg-surface-container-low">
                  <Image
                    src={place.imageUrl || placeholderImage(200, 300, place.title)}
                    alt={place.title}
                    width={320}
                    height={480}
